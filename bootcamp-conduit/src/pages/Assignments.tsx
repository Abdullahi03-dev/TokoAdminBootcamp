...
const Assignments = () => {
  ...
  const fetchCourses = async () => {
    try {
      const res = await axios.get(`${API_URL}/assignments/courses`);
      setCourses(res.data.courses);
    } catch {
      toast.error("Failed to fetch courses");
    }
  ...
  const fetchSubmissions = async () => {
    try {
      const res = await axios.get(`${API_URL}/assignments/submissions`);
      setSubmissions(res.data.submissions);
    } catch {
      toast.error("Failed to fetch submissions");
    }
  ...
  const handleGrade = async (id: number, grade: string) => {
    try {
      await axios.put(`${API_URL}/assignments/grade/${id}?grade=${grade}`);
      toast.success(`Submission graded as ${grade}`);
      fetchSubmissions();
    } catch {
      toast.error("Error grading submission");
  ...
  const handleCreateAssignment = async (e: React.FormEvent) => {
    ...
    try {
      setIsLoading(true);
      await axios.post(
        `${API_URL}/assignments?course_id=${newAssignment.course_id}&title=${newAssignment.title}&instructions=${newAssignment.instructions}`
      );
      toast.success("Assignment created successfully!");
      setIsDialogOpen(false);
      setNewAssignment({ course_id: "", title: "", instructions: "" });
